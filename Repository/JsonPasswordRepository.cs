...
internal class JsonPasswordRepository : IRepository<PasswordModel>
{
    #region Properties and fields

    public string FileName { get; set; } = "data.dat";

    private ISymmetricEncryptionAlgorithm _encryptionAlgorithm;

    #endregion

    #region Constructor

    public JsonPasswordRepository(ISymmetricEncryptionAlgorithm encryptionAlgorithm)
    {
    ...
    {
        return Product.GetApplicationDirectory() is { } applicationDirectory &&
               File.Exists($@"{applicationDirectory}\{FileName}");
    }
